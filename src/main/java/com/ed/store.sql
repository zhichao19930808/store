...
#关联用户表
ALTER TABLE db_store.orders
  ADD CONSTRAINT
  fk_orders_uId
FOREIGN KEY (uId)
REFERENCES db_store.user (id);
...
SELECT *
FROM db_store.orderItem;
#关联商品表
ALTER TABLE db_store.orderItem
  ADD CONSTRAINT
  fk_orderItem_pId
FOREIGN KEY (pId)
REFERENCES db_store.product (id);
