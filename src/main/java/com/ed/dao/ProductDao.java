...
public class ProductDao {
    ...
    public static List<Product> findProduct(String product) {
        ...
        String findAll = "SELECT * FROM db_store.product";
        String findNew = "SELECT * FROM db_store.product ORDER BY pdate DESC LIMIT 9";
        String findHot = "SELECT * FROM db_store.product WHERE is_hot = 1 ORDER BY pdate DESC LIMIT 9";
        try {
            if (product.equals("findAll")) {
                productList = queryRunner.query(findAll, new BeanListHandler<Product>(Product.class));
            }
            if (product.equals("findNew")) {
                productList = queryRunner.query(findNew, new BeanListHandler<Product>(Product.class));
            }
            if (product.equals("findHot")) {
                productList = queryRunner.query(findHot, new BeanListHandler<Product>(Product.class));
            }
